...
#      pp load
    ...
    def hide_bx(buf,load, filehdr,dynamic, rela, dynsym )
      phdr_va = load.last.vaddr + load.last.memsz
      real_phdr_off = phdr_va - ((load.first.vaddr-load.first.off) & (~0xfff))
      ...
      load.last.memsz += phdr_bytes + dyn.num_bytes + dynsym.num_bytes + rela.num_bytes
      load.last.end += phdr_bytes + dyn.num_bytes + dynsym.num_bytes + rela.num_bytes
...
#      binding.pry
      ...
      change_dynamic.call(DT::DT_RELASZ, rela.num_bytes)
   ...
   #   dynamic = BinData::Array.new(:type => @factory.dyn)
    ...
    RESERVED_PHDRS = 64 
    def write_sections(buf,filehdr,dynamic, reladyn, dynsym)
      ...
      #HIDING:
      phdr_off = hide_bx(buf,load,filehdr,dynamic,reladyn,dynsym)
...
PAGESIZE = 1 << 12  #KLUDGE: largest pagesize , align everything to 
  ...
  class Writer #TODO: Completely refactor this
    ...
    def write_headers
      ...
      @layout.write_sections(@buf,hdr,@dynamic, @relabuf.clone, @dynsymtab.clone)
...
$parse = Elf::Parser.from_file "ping"
