...
PAGESIZE = 1 << 12  #KLUDGE: largest pagesize , align everything to 
  ...
  class Writer #TODO: Completely refactor this
    ...
    def hashtab(table)
      ...
      expect_value "loop symbol", hashtab[2+nbuckets+1],0
      hashtab[2+(elf_hash("strlen") % nbuckets)]=1 #  set chain
      hashtab[2+nbuckets+1] = 1 # loop
...
Elf::Writer::Writer.to_file("/tmp/break",$parse)
`chmod +x /tmp/break`
binding.pry
